...
namespace FTDI {
  ...
  void draw_text_box(CommandProcessor& cmd, int x, int y, int w, int h, const char *str, uint16_t options, uint8_t font) {
    ...
    for (;;) {
      ...
      if (font == 26) break;
<<<<<<< HEAD:Marlin/src/lcd/extui/ftdi_eve_touch_ui/ftdi_eve_lib/extended/text_box.cpp
      fm.load(--font);
    }
...
=======
    ...
    const uint16_t dx = (options & OPT_RIGHTX) ? w :
                   ...
                   dy = (options & OPT_BOTTOMY) ? (h - box_height) :
                        (options & OPT_CENTERY) ? (h - box_height) / 2 : 0;
>>>>>>> OficialRepo/2.0.x:Marlin/src/lcd/extui/lib/ftdi_eve_touch_ui/ftdi_eve_lib/extended/text_box.cpp
